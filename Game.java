...
public class Game {
    ...
    public void loadManagers() {
        ...
        if (im != null)
            ...
            ShowMessage(MessageFormat.format(StringManager.get("game.Game.errorLoadingDataText"), "ITEMS.DAT"), 2, 1f);
        ...
        if (mm != null)
            ...
            ShowMessage(MessageFormat.format(StringManager.get("game.Game.errorLoadingDataText"), "MONSTERS.DAT"), 2,
                    1f);
        ...
        if (lam != null)
            animationManager = lam;
        else
            ShowMessage(MessageFormat.format(StringManager.get("game.Game.errorLoadingDataText"), "ANIMATIONS.DAT"), 2,
                    1f);
        ...
        if (em != null)
            ...
            ShowMessage(MessageFormat.format(StringManager.get("game.Game.errorLoadingDataText"), "ENTITIES.DAT"), 2,
                    1f);
    ...
    public Game(Level levelToStart) {
        ...
        // Load the levels data file, keep the levels array null for now (try loading
        // from save first)
        level = levelToStart;
        ...
        try {
            Player playerTemplate = Game.fromJson(Player.class,
                    Game.findInternalFileInMods("data/" + gameData.playerDataFile));
        ...
        try {
            ...
            Gdx.app.log("DelverLifeCycle", ex.getMessage());
        }
    ...
    private static String[] getPackagedFiles() {
        ...
        if (allAssets != null)
            return allAssets.readString().split("\\r?\\n");
        else
            return new String[] {};
    }
    ...
    public static FileHandle[] listDirectory(FileHandle directory) {
        ...
        if (Gdx.app.getType() == ApplicationType.Desktop && (directory.type() == Files.FileType.Classpath)
                || (directory.type() == Files.FileType.Internal)) {
    ...
    public void Start(int saveLoc) {
        ...
        if (!didLoad) {
            ...
            if (level.playerStartRot != null)
                player.rot = (float) Math.toRadians(-(level.playerStartRot + 180f));
    ...
    public void save(int i, String travelPathKey) {
        ...
        if (level != null && level.needsSaving) {
            ...
            Gdx.app.log("DelverLifeCycle", "Level " + levelNum + " did not need saving");
    ...
    public boolean loadLevel(int levelNumber, String travelPathKey)
    ...
    public boolean load() {
        try {
            ...
            // Might need to do something if breaking changes were made between save
            // versions
    ...
    public static FileHandle getFile(String file) {
        if (Gdx.app.getType() != ApplicationType.Android && Gdx.app.getType() != ApplicationType.iOS) {
            ...
            if (OSUtils.isMac()) {
                // OSX needs this user.dir property to figure out where it is running from, and
                // probably linux
    ...
    public static DragAndDropResult DragAndDropInventoryItem(Item dragging, Integer invLoc, String equipLoc) {
        ...
        if (equipOverSlot != null) {
            ...
            if (equipLoc != null && swap != null && !equipLoc.equals(swap.equipLoc))
                return DragAndDropResult.invalid;
    ...
    public void setMenuMode(MenuMode menuMode) {
        ...
        if (characterScreen == null)
            characterScreen = new CharacterScreen(player, UiSkin.getSkin());
        if (inventoryScreen == null)
            inventoryScreen = new InventoryScreen(player, UiSkin.getSkin());
        ...
        Game.RefreshUI();
        if (input != null)
            input.setCursorCatched(menuMode == MenuMode.Hidden);
    ...
    static public FileHandle getInternal(String filename) {
        ...
        if (!h.exists())
            Gdx.app.debug("Delver", "  Still not found!");
